class Whatsapp::TemplateProcessorService
  ...
  def build_header_params(header_data, template)
    ...
    build_media_header_params(normalize_header_data(header_data, template))
  end
  ...
  def build_media_header_params(header_data)
    media_url = header_data['media_url']
    media_type = header_data['media_type']
  ...
  def normalize_header_data(header_data, template)
    return header_data unless header_data.is_a?(Hash)

    normalized = header_data.deep_dup
    media_format = template_media_header_format(template)
    return normalized if media_format.blank?

    normalized['media_type'] = media_format.downcase
    normalized
  end

  def template_media_header_format(template)
    header = template['components']&.find { |component| component['type'] == 'HEADER' }
    format = header&.dig('format')&.upcase
