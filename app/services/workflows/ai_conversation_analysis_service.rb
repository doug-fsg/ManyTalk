...
module Workflows
  class AiConversationAnalysisService
    ...
    def build_payload(data)
      analysis_types = (data['analysis_types'] || []).select do |t|
        Constants::AI_ANALYSIS_TYPES.include?(t)
      end
      analysis_types = Constants::AI_ANALYSIS_TYPES if analysis_types.empty?
      ...
      if destination == 'whatsapp_external'
        output[:whatsapp_inbox_id] = data['whatsapp_inbox_id']
        output[:whatsapp_phone] = data['whatsapp_phone']
      end
